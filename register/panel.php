...
$office_id   = (int)$_SESSION['register_id'];
$office_name = $_SESSION['register_name'] ?? 'Register Office';
...
// Generate a unique registration number: REG-YYYY-DEPTCODE-XXXXX
function generate_reg_number(mysqli $conn, string $dept, int $student_id): string {
    $year = date('Y');

    // Build dept code: take first letter of each word, max 4 chars, uppercase
    ...
    // Find next sequential number for this dept+year
    $stmt = $conn->prepare("
        SELECT COUNT(*) AS cnt FROM students
        WHERE dept = ? AND reg_number LIKE ?
    ");
    $pattern = "REG-{$year}-{$code}-%";
    $stmt->bind_param("ss", $dept, $pattern);
    $stmt->execute();
    $row = $stmt->get_result()->fetch_assoc();
    $stmt->close();

    $seq = str_pad((int)$row['cnt'] + 1, 5, '0', STR_PAD_LEFT);
    return "REG-{$year}-{$code}-{$seq}";
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['student_id'], $_POST['status'])) {
    ...
    if (!in_array($status_disp, ['approved', 'rejected'], true)) {
        ...
        if (!$stepRow) {
            ...
            // If approved, generate & save registration number (only if not already assigned)
            if ($status_disp === 'approved') {
                // Check if already has a reg_number
                $chk = $conn->prepare("SELECT reg_number, dept FROM students WHERE id = ? LIMIT 1");
                $chk->bind_param("i", $student_id);
                $chk->execute();
                $srow = $chk->get_result()->fetch_assoc();
                $chk->close();

                if ($srow && empty($srow['reg_number'])) {
                    $reg_num = generate_reg_number($conn, $srow['dept'], $student_id);
                    $upReg = $conn->prepare("UPDATE students SET reg_number = ? WHERE id = ?");
                    $upReg->bind_param("si", $reg_num, $student_id);
                    $upReg->execute(); $upReg->close();
                    $success = "Approved! Registration number assigned: <strong>" . htmlspecialchars($reg_num) . "</strong>";
                } else {
                    $success = "Status updated to <strong>Approved</strong>. (Registration number already assigned: " . htmlspecialchars($srow['reg_number'] ?? '') . ")";
                }
            } elseif ($status_disp === 'rejected') {
                // If rejecting, clear the reg_number
                $clr = $conn->prepare("UPDATE students SET reg_number = NULL WHERE id = ?");
                $clr->bind_param("i", $student_id);
                $clr->execute(); $clr->close();
                $success = "Student status set to <strong>Rejected</strong>. Registration number cleared.";
            }
        }
    }
}

// Load all students with register + dean status
$list = $conn->query("
    SELECT s.id, s.name, s.gst_roll, s.merit, s.dept, s.reg_number,
           COALESCE(off_s.status, 'waiting') AS off_db_status,
           COALESCE(dean_s.status, 'waiting') AS dean_db_status
    FROM students s
    LEFT JOIN admission_steps off_st ON off_st.step_order = 5 AND off_st.is_active = 1
...
foreach ($all_rows as &$r) {
    ...
    $r['dean_status'] = from_db($r['dean_db_status']);
}
unset($r);
...
$page_title = "Register Office Panel";
...
<style>
  ...
  .panel-avatar { width:52px; height:52px; background:linear-gradient(135deg,#065f46,#059669); border-radius:50%; display:flex; align-items:center; justify-content:center; color:#fff; font-size:22px; }
  ...
  .dept-title { font-size:15px; font-weight:700; background:#f0fff4; padding:8px 14px; border-left:4px solid #059669; border-radius:0 6px 6px 0; margin-bottom:10px; color:#064e3b; }
  .student-row { display:flex; align-items:center; gap:12px; padding:12px 14px; border:1px solid var(--line); border-radius:8px; margin-bottom:8px; background:#fff; transition:box-shadow .15s; flex-wrap:wrap; }
  .student-row:hover { box-shadow:0 3px 10px rgba(0,0,0,.08); }
  .student-info { flex:1; min-width:180px; }
  .student-info .s-name { font-weight:700; font-size:15px; }
  .student-info .s-meta { font-size:12px; color:var(--muted); margin-top:2px; }
  .reg-num-box { font-size:12px; font-weight:700; background:#f0fdf4; border:1px solid #86efac; color:#15803d; padding:4px 10px; border-radius:6px; font-family: monospace; letter-spacing:.5px; }
  .reg-num-empty { font-size:12px; color:#9ca3af; font-style:italic; }
...
<section class="card">
  <div class="panel-header">
    <div class="panel-avatar"><i class="fa-solid fa-building"></i></div>
    <div class="panel-info">
      <h2>Register Office Panel</h2>
          ...
          <div class="student-row">
            <div class="student-info">
              ...
              <div class="s-meta">
                ...
                Merit #<?= (int)$r['merit'] ?> &nbsp;·&nbsp;
                Dean: <span class="badge <?= $r['dean_status'] ?>"><?= ucfirst($r['dean_status']) ?></span>
              </div>
            </div>

            <!-- Registration Number -->
            <div style="min-width:160px;">
              <?php if (!empty($r['reg_number'])): ?>
                <div class="reg-num-box"><i class="fa-solid fa-id-card"></i> <?= htmlspecialchars($r['reg_number']) ?></div>
              <?php else: ?>
                <span class="reg-num-empty">No reg. number yet</span>
              <?php endif; ?>
            </div>

            <span class="badge <?= $r['off_status'] ?>">Register: <?= ucfirst($r['off_status']) ?></span>
